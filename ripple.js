let gl;
let canvas;
let gridSizeX, gridSizeY, windowOffsetX, windowOffsetY;
let windowWidth, windowHeight, viewAngle, viewHeight;
let sim;
let damping;
...
let shaderProgramMain, shaderProgram3D, shaderProgramDefault, shaderProgramSingletTE, shaderProgramAcoustic,
    shaderProgramEMFDTD, shaderProgramFDTD_Magnet, shaderProgramFDTD_Electron, shaderProgramFDTD_Polar,
    shaderProgramDraw, shaderProgramDrawLine, shaderProgramMode, shaderProgramPoke;

function initShader(fs, vs, prefix) {
    const fragmentShader = getShader(gl, fs, prefix);
//        var vs = (fs == "shader-draw-fs" || fs == "shader-mode-fs") ? "shader-draw-vs" : "shader-vs";
    const vertexShader = getShader(gl, vs, prefix);
    ...
    shaderProgram.fieldSamplerUniform = gl.getUniformLocation(shaderProgram, "fieldSampler");
    // 在初始化着色器时添加 磁/电/极化 纹理的uniform位置
    shaderProgram.magneticSamplerUniform = gl.getUniformLocation(shaderProgram, "magneticSampler");
    shaderProgram.electricSamplerUniform = gl.getUniformLocation(shaderProgram, "electricSampler");
    shaderProgram.polarSamplerUniform = gl.getUniformLocation(shaderProgram, "polarSampler");

    // 在初始化着色器时添加材料参数纹理的uniform位置
    shaderProgram.materialSamplerUniform = gl.getUniformLocation(shaderProgram, "materialParameters");
...
function initShaders() {
    ...
    shaderProgramFDTD_Magnet = initShader("shader-FDTD-Magnet-fs", "shader-vs", null);
    shaderProgramFDTD_Magnet.stepSizeXUniform = gl.getUniformLocation(shaderProgramFDTD_Magnet, "stepSizeX");
    shaderProgramFDTD_Magnet.stepSizeYUniform = gl.getUniformLocation(shaderProgramFDTD_Magnet, "stepSizeY");

    shaderProgramFDTD_Electron = initShader("shader-FDTD-Electron-fs", "shader-vs", null);
    shaderProgramFDTD_Electron.stepSizeXUniform = gl.getUniformLocation(shaderProgramFDTD_Electron, "stepSizeX");
    shaderProgramFDTD_Electron.stepSizeYUniform = gl.getUniformLocation(shaderProgramFDTD_Electron, "stepSizeY");

    shaderProgramFDTD_Polar = initShader("shader-FDTD-Polar-fs", "shader-vs", null);
    shaderProgramFDTD_Polar.stepSizeXUniform = gl.getUniformLocation(shaderProgramFDTD_Polar, "stepSizeX");
    shaderProgramFDTD_Polar.stepSizeYUniform = gl.getUniformLocation(shaderProgramFDTD_Polar, "stepSizeY");
...
let rrtMaterialParas; // 存储折射率和阻尼的纹理

let mvMatrix = mat4.create();
const mvMatrixStack = [];
const pMatrix = mat4.create();
const matrix3d = mat4.create();
let zoom3d = 1;
...
let rrtMainField1, rrtMainField2;
let rrtParallelField2, rrtParallelField1;
let rrtPolarField1, rrtPolarField2;
let fbType;
...
var colorBuffer;
var colors;
// defined sub_simulate function for dual-updating case when applying standard FDTD method for EM
function simulate() {
    if (sim.theoryID === 0)
        scalar_simulate(shaderProgramDefault);
    else if (sim.theoryID === 1)
        scalar_simulate(shaderProgramSingletTE);
    else
        compute_main_field();
    if (sim.theoryID === 2||sim.theoryID === 3) {
        compute_polar_field();
        compute_parallel_field();
    }
}

function compute_main_field() {
    const rt = rrtMainField1;
    rrtMainField1 = rrtMainField2;
    rrtMainField2 = rt;

    const rrt_fb_Field1 = rrtMainField1.framebuffer;
    gl.bindFramebuffer(gl.FRAMEBUFFER, rrt_fb_Field1);

    let prog;
    prog = shaderProgramFDTD_Magnet;
    // var prog = sim.acoustic ? shaderProgramAcoustic : shaderProgramDefault;
    gl.useProgram(prog);
    gl.viewport(0, 0, rrt_fb_Field1.width, rrt_fb_Field1.height);
    ...
    gl.bindTexture(gl.TEXTURE_2D, rrtMainField2.texture);
    gl.uniform1i(prog.magneticSamplerUniform, 0);
    gl.activeTexture(gl.TEXTURE1); // 使用纹理单元1
    gl.bindTexture(gl.TEXTURE_2D, rrtParallelField1.texture);
    gl.uniform1i(prog.electricSamplerUniform, 1);

    gl.uniform1f(prog.stepSizeXUniform, 1/gridSizeX);
    gl.uniform1f(prog.stepSizeYUniform, 1/gridSizeY);

    gl.activeTexture(gl.TEXTURE2); // 使用纹理单元2
    gl.bindTexture(gl.TEXTURE_2D, rrtMaterialParas.texture);
    gl.uniform1i(prog.materialSamplerUniform, 2);
...
function compute_parallel_field() {
    const rt = rrtParallelField1;
    rrtParallelField1 = rrtParallelField2;
    rrtParallelField2 = rt;

    const rrt_fb_Field1 = rrtParallelField1.framebuffer;
    gl.bindFramebuffer(gl.FRAMEBUFFER, rrt_fb_Field1);

    let prog;
    prog = shaderProgramFDTD_Electron;
    // var prog = sim.acoustic ? shaderProgramAcoustic : shaderProgramDefault;
    gl.useProgram(prog);
    gl.viewport(0, 0, rrt_fb_Field1.width, rrt_fb_Field1.height);
    gl.clearColor(0.0, 0.0, 0.0, 0.0);
    ...
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, rrtParallelField2.texture);
    gl.uniform1i(prog.electricSamplerUniform, 0);
    gl.activeTexture(gl.TEXTURE1); // 使用纹理单元1
    gl.bindTexture(gl.TEXTURE_2D, rrtMainField1.texture);
    gl.uniform1i(prog.magneticSamplerUniform, 1);
    gl.activeTexture(gl.TEXTURE2); // 使用纹理单元2
    gl.bindTexture(gl.TEXTURE_2D, rrtPolarField1.texture);
    gl.uniform1i(prog.polarSamplerUniform, 2);

    gl.uniform1f(prog.stepSizeXUniform, 1/gridSizeX);
    gl.uniform1f(prog.stepSizeYUniform, 1/gridSizeY);

    gl.activeTexture(gl.TEXTURE3); // 使用纹理单元3
    gl.bindTexture(gl.TEXTURE_2D, rrtMaterialParas.texture);
    gl.uniform1i(prog.materialSamplerUniform, 3);
...
function compute_polar_field() {
    const rt = rrtPolarField1;
    rrtPolarField1 = rrtPolarField2;
    rrtPolarField2 = rt;

    const rrt_fb_Field1 = rrtPolarField1.framebuffer;
    gl.bindFramebuffer(gl.FRAMEBUFFER, rrt_fb_Field1);

    let prog;
    prog = shaderProgramFDTD_Polar;
    // var prog = sim.acoustic ? shaderProgramAcoustic : shaderProgramDefault;
    gl.useProgram(prog);
    gl.viewport(0, 0, rrt_fb_Field1.width, rrt_fb_Field1.height);
    gl.clearColor(0.0, 0.0, 0.0, 0.0);
    ...
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, rrtPolarField2.texture);
    gl.uniform1i(prog.polarSamplerUniform, 0);
...
function scalar_simulate(prog) {
    ...
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, rrtMainField2.texture);
    gl.uniform1i(prog.fieldSamplerUniform, 0);
    gl.uniform1f(prog.stepSizeXUniform, 1/gridSizeX);
    gl.uniform1f(prog.stepSizeYUniform, 1/gridSizeY);
...
function drawPoke(x, y, v) {
    var rt = rrtMainField1;
    rrtMainField1 = rrtMainField2;
    rrtMainField2 = rt;

    var rttFramebuffer = rrtMainField1.framebuffer;
    var rttTexture = rrtMainField1.texture;
    gl.bindFramebuffer(gl.FRAMEBUFFER, rttFramebuffer);
...
function drawLineSource(x, y, x2, y2, f, gauss) {
    ...
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.ONE, gl.ONE); // always add
    // 传递统一变量到片元着色器
...
function setupForParametersDrawing(v, k) {
    if (sim.drawingSelection > 0) {
        ...
        var rttFramebuffer = rrtMaterialParas.framebuffer;
        gl.bindFramebuffer(gl.FRAMEBUFFER, rttFramebuffer);
        gl.viewport(0, 0, rttFramebuffer.width, rttFramebuffer.height);
        gl.useProgram(shaderProgramDraw);
        ...
        gl.vertexAttrib4f(shaderProgramDraw.colorAttribute, 0.0, 0.0, v, k);
    }
}
...
function drawSolidEllipse(cx, cy, xr, yr, med, k) {
    setupForParametersDrawing(med, k);
    gl.bindBuffer(gl.ARRAY_BUFFER, sourceBuffer);
    var coords = [cx, cy];
    var i;
    for (i = -xr; i <= xr; i++) {
...
function drawScene(bright) {
    ...
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, rrtMainField1.texture);
    gl.uniform1i(shaderProgramMain.fieldSamplerUniform, 0);
    ...
    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, rrtMaterialParas.texture);
    gl.uniform1i(gl.getUniformLocation(shaderProgramMain, "materialParameters"), 1);
...
function drawScene3D(bright) {
    ...
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, rrtMainField1.texture);
    gl.uniform1i(shaderProgram3D.fieldSamplerUniform, 0);
    ...
    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, rrtMaterialParas.texture);
    gl.uniform1i(gl.getUniformLocation(shaderProgram3D, "materialParameters"), 1);
...
document.passCanvas = function passCanvas (cv, sim_) {
    ...
    // 再初始化参数纹理
    rrtMainField2 = initTextureFramebuffer();
    if (!rrtMainField2) {
    ...
    rrtMainField1 = initTextureFramebuffer();
    rrtParallelField1 = initTextureFramebuffer()
    rrtParallelField2 = initTextureFramebuffer()
    rrtPolarField1 = initTextureFramebuffer()
    rrtPolarField2 = initTextureFramebuffer()
    rrtMaterialParas = initTextureFramebuffer();
    initShaders();
    initBuffers();
    initTextures();  // ???
...
//    	drawWalls(rrtMainField1);
    ...
    sim.setResolution = function (x, y, wx, wy) {
        ...
        deleteRenderTexture(rrtMainField1);
        deleteRenderTexture(rrtMainField2);
        deleteRenderTexture(rrtParallelField1);
        deleteRenderTexture(rrtParallelField2);
        deleteRenderTexture(rrtPolarField1);
        deleteRenderTexture(rrtPolarField2);
        deleteRenderTexture(rrtMaterialParas);
        rrtMainField2 = initTextureFramebuffer();
        rrtMainField1 = initTextureFramebuffer();
        rrtParallelField2 = initTextureFramebuffer();
        rrtParallelField1 = initTextureFramebuffer();
    ...
    sim.drawSolidEllipse = function (x, y, x2, y2, m, k) { drawSolidEllipse(x, y, x2, y2, m, k); }
    sim.drawMedium = function (x, y, x2, y2, x3, y3, x4, y4, m, m2, k1, k2) { drawMedium(x, y, x2, y2, x3, y3, x4, y4, m, m2, k1, k2); }
    sim.drawPML = function (x1, y1, x2, y2, x3, y3, x4, y4, med, k_min, f, n) { drawPML(x1, y1, x2, y2, x3, y3, x4, y4, med, k_min, f, n); }
    ...
    sim.doBlank = function () {
        function blankFramebuffer(rttFramebuffer) {
            gl.bindFramebuffer(gl.FRAMEBUFFER, rttFramebuffer);
            gl.viewport(0, 0, rttFramebuffer.width, rttFramebuffer.height);
            gl.colorMask(true, true, true, true);
            gl.clearColor(0.0, 0.0, 0.0, 0.0);
        ...
        // 清除第一个和第二个帧缓冲区
        blankFramebuffer(rrtMainField1.framebuffer);
        blankFramebuffer(rrtParallelField1.framebuffer);
        blankFramebuffer(rrtPolarField1.framebuffer);
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    };
